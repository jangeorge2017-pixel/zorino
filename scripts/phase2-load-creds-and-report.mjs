...
if (url && serviceKey) {
  const res = await fetch(
    `${url}/rest/v1/integration_settings?select=key,value&key=in.(ALIEXPRESS_APP_KEY,ALIEXPRESS_APP_SECRET,ALIEXPRESS_TRACKING_ID)`,
    {
      headers: {
        apikey: serviceKey,
  ...
  if (res.ok) {
    ...
    console.log(
      "integration_settings rows:",
      rows.map((r) => ({ key: r.key, status: r.value?.trim() ? "SET" : "EMPTY" }))
    );
  } else {
    console.log("integration_settings query failed:", res.status, await res.text());
  }
} else {
  console.log("Supabase not configured for credential hydrate");
}
...
async function search(query, extraBiz = {}) {
  const biz = {
    keywords: query.trim(),
    page_no: "1",
    page_size: "24",
  ...
  const mapped = raw.map((p) => ({
    ...
    affiliateLink: (p.promotion_link || p.product_detail_url || null)?.slice(0, 80),
...
const outPath = resolve(process.cwd(), "scripts/output/phase2-aliexpress-search-report.json");
...
const summary = reports.map((r) => ({
  ...
  topRelevant: (r.relevantMapped ?? []).slice(0, 8).map((p) => ({
    title: p.title,
    price: p.price,
  })),
  topIrrelevantSample: (r.irrelevantTitles ?? []).slice(0, 5),
}));

console.log(JSON.stringify({ summary, reportFile: outPath }, null, 2));
